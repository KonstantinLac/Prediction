    ...
    class PredictionM
    {
        ...
        public double[] GetPolyPrediction(double[] coef1, int number)
        {
            if (value_garmoniks == 0)
            {
                ...
                double[] U = new double[coef.Count + number];
                a01 = 0;
                ...
                double dNiz = 0;
                double d = 0;
                darbi = new double[(coef.Count / 2)];

                for (int j = 0; j < (coef.Count / 2); j++)
                {
                ...
                for (int i = 0; i < coef.Count + (number); i++)
                {
                    U[i] = arr_x[0, i];
                }
                return U;
            ...
            else
            {
                ...
                for (int j = arr_out.Length - number; j < arr_out.Length; j++)
                {
                    arr_out[j] = a01 + (ak1[value_garmoniks] * Math.Cos(2 * Math.PI * value_garmoniks * (j) / coef1.Length) + bk1[value_garmoniks] * Math.Sin(2 * Math.PI * value_garmoniks * (j) / coef1.Length));
                }
                return arr_out;
            }
        }
